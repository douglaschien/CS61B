...
public class SList {
  ...
  public void squish() {
    // Fill in your solution here.  (Ours is eleven lines long.)   不能把人家head改球了。

    if ((!this.isEmpty())&&(this.head.next!=null)) {
        SListNode prevNode=head;     //这些叫做 local variables 当method execute结束后 就丢弃。
        SListNode currNode=prevNode.next;
        while(currNode!=null){
 ...
 public void twin() {
    ...
    // SListNode d = c;
    while(c!=null){
      SListNode node = new SListNode(c.item);
      node.next=c.next;
      c.next=node;
      c=node.next;
    }
